/*
 * arrange.c — pure-C arrange stage with CLI integration.
 *
 * Decodes a video with libav, runs the greedy block solver, extracts features
 * (imgops), matches against the library (match.c), and writes manifests.
 *
 * Usage:
 *   arrange --video in.mp4 --features features.bin --registry registry.bin \
 *          --manifests out_dir [--bits G] [--no-edges] [--scales 32,64,128] \
 *          [--max-block-pct 0.5] [--hero-min-pct 0.0625] [--max-frames 0] \
 *          [--threads N] [--verbose] [--quiet] [--json]
 */
#include "badapple.h"
#include "imgops.h"
#include "video.h"
#include "cli.h"

#include <time.h>
#include <sys/stat.h>
#include <errno.h>
#ifdef _OPENMP
#include <omp.h>
#endif

static int g_G = 1, g_has_edges = 1, g_feat_len = 0;
static int g_scales[16] = {32, 64, 128};
static int g_n_scales = 3;
static double g_max_block_pct = 0.5, g_hero_min_pct = 0.0833;

typedef struct {
    int32_t x, y, w, h, op_id, page_idx;
} Inst;

/* ── Compact hash+pid cache: 16 bytes/entry, near-zero lookup cost ─
 * Instead of storing full feature bytes (43 KB each), we store only a
 * 64-bit FNV-1a hash + pid.  The full-feature hash uses a sparse 64-byte
 * sample across all scale levels.  With ~2 000 library pages the collision
 * probability is < 10⁻¹³ — effectively zero.
 *
 * Open addressing with linear probing, power-of-2 capacity (bitmask),
 * 75% load-factor resize, bounded 32-step probe. */
typedef struct { uint64_t hash; int32_t pid; } CacheSlot;

static CacheSlot *g_ccache = NULL;  /* coarse cache (keyed on 1 KB feature) */
static size_t    g_ccache_cap = 0, g_ccache_n = 0;

static CacheSlot *g_fcache = NULL;  /* full cache (keyed on full feature) */
static size_t    g_fcache_cap = 0, g_fcache_n = 0;

/* ── Hash functions ─────────────────────────────────────────────── */
static uint64_t fnv1a_64(const uint8_t *d, int len) {
    uint64_t h = 14695981039346656037ULL;
    for (int i = 0; i < len; i++) { h ^= d[i]; h *= 1099511628211ULL; }
    return h | 1;  /* 0 = empty sentinel */
}

/* Full feature: sparse 64-byte sample across all scales (≤64 ns/tile). */
static uint64_t full_feat_hash(const uint8_t *feat, int feat_len) {
    uint64_t h = 14695981039346656037ULL;
    int n = feat_len < 64 ? feat_len : 64;
    int step = feat_len / n;
    if (step < 1) step = 1;
    for (int i = 0; i < n; i++) {
        h ^= feat[(size_t)i * step];
        h *= 1099511628211ULL;
    }
    return h | 1;
}

/* Coarse feature (≤ 1 KB): hash all bytes (~1 μs). */
static uint64_t coarse_feat_hash(const uint8_t *feat, int len) {
    return fnv1a_64(feat, len);
}

/* ── Cache slot primitives ──────────────────────────────────────── */
#define CACHE_PROBE_MAX 32

static inline int cache_slot_lookup(const CacheSlot *tab, size_t cap,
                                    uint64_t h, int *pid) {
    if (cap == 0) return 0;
    size_t mask = cap - 1, idx = h & mask;
    for (size_t i = 0; i < CACHE_PROBE_MAX && i < cap; i++) {
        if (tab[idx].hash == 0) return 0;          /* empty → miss */
        if (tab[idx].hash == h) { *pid = tab[idx].pid; return 1; }
        idx = (idx + 1) & mask;
    }
    return 0;
}

static void cache_slot_grow(CacheSlot **tab, size_t *cap) {
    size_t ncap = *cap ? *cap * 2 : 2048;
    CacheSlot *nt = (CacheSlot *)calloc(ncap, sizeof(CacheSlot));
    if (!nt) return;
    if (*tab) {
        size_t mask = ncap - 1;
        for (size_t i = 0; i < *cap; i++) {
            if ((*tab)[i].hash != 0) {
                size_t idx = (*tab)[i].hash & mask;
                while (nt[idx].hash != 0) idx = (idx + 1) & mask;
                nt[idx] = (*tab)[i];
            }
        }
        free(*tab);
    }
    *tab = nt; *cap = ncap;
}

static void cache_slot_put(CacheSlot **tab, size_t *cap, size_t *n,
                           uint64_t h, int pid) {
    if (*n + 1 > *cap * 3 / 4)          /* resize at 75 % load */
        cache_slot_grow(tab, cap);
    size_t mask = *cap - 1, idx = h & mask;
    while ((*tab)[idx].hash != 0) idx = (idx + 1) & mask;
    (*tab)[idx].hash = h;
    (*tab)[idx].pid  = pid;
    (*n)++;
}

/* ── Public cache API (same signatures as before) ───────────────── */
static int coarse_cache_lookup(const uint8_t *feat, int feat_len, int *pid) {
    return cache_slot_lookup(g_ccache, g_ccache_cap,
                             coarse_feat_hash(feat, feat_len), pid);
}
static void coarse_cache_put(const uint8_t *feat, int feat_len, int pid) {
    cache_slot_put(&g_ccache, &g_ccache_cap, &g_ccache_n,
                   coarse_feat_hash(feat, feat_len), pid);
}
static int cache_lookup(const uint8_t *feat, int *pid) {
    return cache_slot_lookup(g_fcache, g_fcache_cap,
                             full_feat_hash(feat, g_feat_len), pid);
}
static void cache_put(const uint8_t *feat, int pid) {
    cache_slot_put(&g_fcache, &g_fcache_cap, &g_fcache_n,
                   full_feat_hash(feat, g_feat_len), pid);
}

typedef struct { double read, gray, solve, feat, match, write; long tiles, hits; } Timings;

typedef struct { int x, y, w, h, manifest_idx; } TileSpec;

typedef struct {
    int64_t *sum;
    uint8_t *visited;
    TileSpec *specs;
    int *coarse_hit;
    uint8_t *crop_bufs;
    int *miss_idx;
    int *results;
    int cap_w, cap_h;
    int spec_cap;
    int coarse_cap;
    int miss_cap;
    int result_cap;
    size_t crop_cap;
} SolveBuffers;

/* Forward declaration for solve_full (defined after main). */
void solve_full(const uint8_t *gray, const uint8_t *color_pixels, int color_stride, int color_channels,
                int w, int h,
                const FeatureDB *db, const Registry *reg,
                int pid_white, int pid_black, int max_block, int hero_min,
                Inst *manifest, int *nout, uint8_t *tiles, int *ntiles, Timings *t,
                SolveBuffers *bufs);

static void print_help(void) {
    fprintf(stderr,
        "videomatch arrange — match video frames to a source library\n"
        "\n"
        "Usage:\n"
        "  arrange --video <file> --features <file> --registry <file> \\\n"
        "         --manifests <dir> [options]\n"
        "\n"
        "Required:\n"
        "  --video <file>          Input video file\n"
        "  --features <file>       Library features.bin\n"
        "  --registry <file>       Library registry.bin\n"
        "  --manifests <dir>       Output directory for manifests\n"
        "\n"
        "Options:\n"
        "  --bits <n>              Bits per cell, 1-8 (default: 1)\n"
        "  --no-edges              Disable edge detection features\n"
        "  --scales <list>         Comma-separated scale levels (default: 32,64,128)\n"
        "  --max-block-pct <f>     Max block size as fraction of width (default: 0.5)\n"
        "  --hero-min-pct <f>      Min hero block as fraction of height (default: 0.0625)\n"
        "  --max-frames <n>        Process only N frames (0 = all)\n"
        "  --threads <n>           Thread count for matching (0 = auto)\n"
        "  --verbose, -v           Verbose output\n"
        "  --quiet, -q             Suppress non-error output\n"
        "  --json                  JSON output mode\n"
        "  --help, -h              Show this help\n"
        "\n"
        "  Note: When using mise, prefix flags with `--` to avoid interception:\n"
        "    mise run arrange -- --help\n"
        "    mise run arrange -- --max-frames 100\n"
        "    mise run arrange -- --threads 4\n"
        "\n"
        "Output:\n"
        "  manifests/<frame>.bin    Binary manifests (24-byte records)\n"
        "  Progress: frame N/M | FPS | cache hit%%\n"
    );
}

int arrange_main(int argc, char **argv) {
    const char *video_path = cli_opt_str("video", NULL);
    const char *feat_path  = cli_opt_str("features", "features.bin");
    const char *reg_path   = cli_opt_str("registry", "registry.bin");
    const char *man_dir    = cli_opt_str("manifests", "manifests_greedy");

    if (cli_has("help") || cli_has("h") || !video_path) {
        print_help();
        return video_path ? 0 : 1;
    }

    if (cli_has("bits")) g_G = cli_opt_int("bits", 1);
    if (cli_has("no-edges")) g_has_edges = 0;
    if (cli_has("scales")) {
        /* Parse comma-separated scale list, e.g., "32,64,128" */
        const char *sstr = cli_opt_str("scales", "32,64,128");
        g_n_scales = 0;
        const char *p = sstr;
        while (*p && g_n_scales < 16) {
            while (*p == ' ') p++;
            g_scales[g_n_scales++] = atoi(p);
            while (*p && *p != ',') p++;
            if (*p == ',') p++;
        }
    }
    if (cli_has("max-block-pct")) g_max_block_pct = cli_opt_dbl("max-block-pct", 0.5);
    if (cli_has("hero-min-pct")) g_hero_min_pct = cli_opt_dbl("hero-min-pct", 0.0625);
    int max_frames = cli_opt_int("max-frames", 0);
    int threads = cli_opt_int("threads", 0);
    cli_set_threads(threads);

    FeatureDB db; Registry reg;
    if (ba_load_features(feat_path, &db) != 0) {
        cli_die("cannot load features: %s", feat_path);
    }
    if (ba_load_registry(reg_path, &reg) != 0) {
        cli_die("cannot load registry: %s", reg_path);
    }
    /* Override global params from library header to ensure consistency */
    g_G = db.G; g_feat_len = db.feat_len;
    g_n_scales = db.n_scales;
    for (int i = 0; i < db.n_scales; i++) g_scales[i] = db.scales[i];
    g_has_edges = db.has_edges;

    cli_info("library: %d pages | scales=%d G=%d edges=%d | feat_len=%d",
             db.n_pages, db.n_scales, db.G, db.has_edges, db.feat_len);

    VideoDecoder *vd = video_decoder_open(video_path);
    if (!vd) cli_die("cannot open video: %s", video_path);

    int fw = video_decoder_width(vd), fh = video_decoder_height(vd);
    double source_fps = video_decoder_fps(vd);
    int total_frames = (int)(source_fps * 300.0); /* estimate */
    cli_info("video: %dx%d | fps=%.1f | estimating %d frames",
             fw, fh, source_fps, total_frames);

    /* Hero pages: mean intensity of the coarsest-scale gray feature.
     * The first sub-feature is gray at scales[0]×scales[0]. */
    int hero_feat_len = g_scales[0] * g_scales[0];  /* just the first gray block */
    int pid_white = 0, pid_black = 0;
    double mx = -1, mn = 256;
    for (int i = 0; i < db.n_pages; i++) {
        double s = 0;
        const uint8_t *f = db.data + (size_t)i * db.feat_len;
        for (int j = 0; j < hero_feat_len; j++) s += f[j];
        double m = s / hero_feat_len;
        if (m > mx) { mx = m; pid_white = i; }
        if (m < mn) { mn = m; pid_black = i; }
    }

    int max_block = (int)(fw * g_max_block_pct);
    max_block = (max_block / 8) * 8;
    if (max_block < 8) max_block = 8;
    int hero_min = (int)(fh * g_hero_min_pct);

    if (mkdir(man_dir, 0755) != 0 && errno != EEXIST) {
        cli_die("cannot create output directory: %s", man_dir);
    }

    /* Write fps sidecar so render can auto-detect source frame rate */
    char fps_path[1024];
    snprintf(fps_path, sizeof(fps_path), "%s/fps.bin", man_dir);
    FILE *fpsf = fopen(fps_path, "wb");
    if (fpsf) {
        fwrite(&source_fps, sizeof(double), 1, fpsf);
        fclose(fpsf);
    }

    match_set_threads(g_cli.threads);
    Timings t = {0};
    int frames_done = 0, total_tiles = 0;
    clock_t start = clock();

    Img frame;
    Img gray = {0};
    size_t gray_sz = 0;
    Inst *manifest = NULL;
    uint8_t *tiles = NULL;
    size_t manifest_cap = 0;
    SolveBuffers sbufs = {0};
    int fi = 0;
    while (video_decoder_next(vd, &frame) == 1 && (max_frames == 0 || fi < max_frames)) {
        clock_t tr = clock();
        size_t needed_gray = (size_t)fw * fh;
        if (!gray.pixels || gray_sz != needed_gray) {
            img_free(&gray);
            img_to_gray(&frame, &gray);
            gray_sz = needed_gray;
        } else if (frame.channels == 1) {
            memcpy(gray.pixels, frame.pixels, needed_gray);
        } else {
            const uint8_t *s = frame.pixels;
            uint8_t *d = gray.pixels;
            for (int y = 0; y < fh; y++) {
                for (int x = 0; x < fw; x++) {
                    const uint8_t *p = s + (size_t)y * frame.stride + x * 3;
                    int v = (29 * p[0] + 150 * p[1] + 77 * p[2] + 128) >> 8;
                    d[(size_t)y * fw + x] = (uint8_t)v;
                }
            }
        }
        t.gray += (double)(clock() - tr) / CLOCKS_PER_SEC;

        size_t needed_cap = (size_t)((fw+7)/8) * ((fh+7)/8) + 1;
        if (needed_cap > manifest_cap) {
            size_t ncap = needed_cap * 2;
            Inst *tmp_m = (Inst *)realloc(manifest, ncap * sizeof(Inst));
            if (!tmp_m) { free(manifest); manifest = NULL; cli_die("out of memory"); }
            manifest = tmp_m;
            uint8_t *tmp_t = (uint8_t *)realloc(tiles, (size_t)db.feat_len * ncap);
            if (!tmp_t) { free(tiles); tiles = NULL; cli_die("out of memory"); }
            tiles = tmp_t;
            manifest_cap = ncap;
        }
        int n = 0, nt = 0;
        solve_full(gray.pixels, frame.pixels, frame.stride, frame.channels, fw, fh,
                   &db, &reg, pid_white, pid_black,
                   max_block, hero_min, manifest, &n, tiles, &nt, &t, &sbufs);

        for (int k = 0; k < n; k++) if (manifest[k].op_id >= 0) total_tiles++;

        char fn[1024]; snprintf(fn, sizeof(fn), "%s/%04d.bin", man_dir, fi);
        {
            size_t total = 3 * sizeof(uint32_t) + (size_t)n * 6 * sizeof(int32_t);
            uint8_t *mbuf = (uint8_t *)malloc(total);
            if (mbuf) {
                uint32_t *hp = (uint32_t *)mbuf;
                hp[0] = (uint32_t)fw;
                hp[1] = (uint32_t)fh;
                hp[2] = (uint32_t)n;
                int32_t *rp = (int32_t *)(hp + 3);
                for (int k = 0; k < n; k++) {
                    rp[0] = manifest[k].x;
                    rp[1] = manifest[k].y;
                    rp[2] = manifest[k].w;
                    rp[3] = manifest[k].h;
                    rp[4] = manifest[k].op_id;
                    rp[5] = manifest[k].page_idx;
                    rp += 6;
                }
                FILE *f = fopen(fn, "wb");
                if (f) {
                    fwrite(mbuf, 1, total, f);
                    fclose(f);
                }
                free(mbuf);
            }
        }
        img_free(&frame);

        frames_done++; fi++;
        if (!g_cli.quiet) {
            double el = (double)(clock() - start) / CLOCKS_PER_SEC;
            double fps = frames_done / (el > 0 ? el : 1);
            long detail = t.tiles + t.hits;
            double hit = detail ? 100.0 * t.hits / detail : 0;
            cli_progress_frame("arrange", fi, max_frames > 0 ? max_frames : total_frames,
                               fps, hit);
        }
    }
    free(manifest);
    free(tiles);
    free(sbufs.sum);
    free(sbufs.visited);
    free(sbufs.specs);
    free(sbufs.coarse_hit);
    free(sbufs.crop_bufs);
    free(sbufs.miss_idx);
    free(sbufs.results);
    img_free(&gray);
    video_decoder_close(vd);
    ba_free_features(&db);
    ba_free_registry(&reg);

    double total_time = (double)(clock() - start) / CLOCKS_PER_SEC;
    double overall_fps = frames_done / (total_time > 0.0 ? total_time : 1.0);
    long detail = t.tiles + t.hits;
    double hit_rate = detail ? 100.0 * t.hits / detail : 0.0;

    char summary[256];
    snprintf(summary, sizeof(summary),
             "arrange complete in %.2fs | %.2f fps | %d tiles | cache %.1f%%",
             total_time, overall_fps, total_tiles, hit_rate);
    cli_progress_done(summary);

    if (g_cli.verbose) {
        cli_info("phase breakdown:");
        cli_info("  gray    : %.3fs", t.gray);
        cli_info("  solve   : %.3fs", t.solve);
        cli_info("  feat    : %.3fs", t.feat);
        cli_info("  match   : %.3fs", t.match);
        cli_info("  write   : %.3fs", t.write);
        cli_info("  tiles   : %ld", t.tiles);
        cli_info("  hits    : %ld", t.hits);
    }

    if (g_cli.json) {
        char jbuf[64];
        cli_json_start();
        cli_json_field("stage", "arrange");
        snprintf(jbuf, sizeof(jbuf), "%d", frames_done);
        cli_json_field("frames", jbuf);
        snprintf(jbuf, sizeof(jbuf), "%.2f", overall_fps);
        cli_json_field("fps", jbuf);
        snprintf(jbuf, sizeof(jbuf), "%d", total_tiles);
        cli_json_field("tiles", jbuf);
        cli_json_end();
    }

    return 0;
}

/* ── Pool allocator for tile feature buffers ─────────────────────── */
typedef struct {
    uint8_t *buf;    /* single contiguous buffer */
    size_t   cap;    /* total capacity in bytes */
    size_t   used;   /* current write offset in bytes */
    int      count;  /* number of allocations made */
} TilePool;

static void pool_init(TilePool *p, size_t cap) {
    p->buf = (uint8_t *)malloc(cap);
    if (!p->buf) { p->cap = 0; p->used = 0; p->count = 0; return; }
    p->cap = cap;
    p->used = 0;
    p->count = 0;
}
static uint8_t *pool_alloc(TilePool *p, size_t sz) {
    if (p->used + sz > p->cap) {
        /* grow by 2× or needed amount */
        size_t ncap = p->cap * 2;
        if (ncap < p->used + sz) ncap = p->used + sz;
        uint8_t *nb = (uint8_t *)realloc(p->buf, ncap);
        if (!nb) return NULL;
        p->buf = nb;
        p->cap = ncap;
    }
    uint8_t *ptr = p->buf + p->used;
    p->used += sz;
    p->count++;
    return ptr;
}
static void pool_reset(TilePool *p) {
    p->used = 0;
    p->count = 0;
}
static void pool_free(TilePool *p) {
    free(p->buf);
    p->buf = NULL; p->cap = 0; p->used = 0; p->count = 0;
}

void solve_full(const uint8_t *gray, const uint8_t *color_pixels_param, int color_stride_param, int color_channels_param,
                int w, int h,
                const FeatureDB *db, const Registry *reg,
                int pid_white, int pid_black, int max_block, int hero_min,
                Inst *manifest, int *nout, uint8_t *tiles, int *ntiles, Timings *t,
                SolveBuffers *bufs) {
    const uint8_t *color_pixels = color_pixels_param;
    int color_stride = color_stride_param;
    int color_channels = color_channels_param;
    const int CELL = 8;

    /* Reallocate sum/visited buffers only when dimensions grow */
    if (bufs->cap_w < w || bufs->cap_h < h) {
        free(bufs->sum);
        free(bufs->visited);
        bufs->sum = NULL;
        bufs->visited = NULL;
        bufs->cap_w = w;
        bufs->cap_h = h;
    }
    size_t sum_needed = (size_t)(w + 1) * (h + 1);
    if (!bufs->sum) {
        bufs->sum = (int64_t *)malloc(sum_needed * sizeof(int64_t));
        if (!bufs->sum) { *nout = 0; *ntiles = 0; return; }
    }
    int gh = (h + CELL - 1) / CELL, gw = (w + CELL - 1) / CELL;
    size_t vis_needed = (size_t)gh * gw;
    if (!bufs->visited) {
        bufs->visited = (uint8_t *)malloc(vis_needed);
        if (!bufs->visited) { *nout = 0; *ntiles = 0; return; }
    }

    int64_t *sum = bufs->sum;
    /* Zero first row and first column for integral image */
    memset(sum, 0, (size_t)(w + 1) * sizeof(int64_t));
    for (int y = 1; y <= h; y++) sum[y * (w + 1)] = 0;
    /* Single-pass threshold + integral image (eliminates binary buffer) */
    for (int y = 0; y < h; y++) {
        int64_t rowsum = 0;
        for (int x = 0; x < w; x++) {
            int val = (gray[(size_t)y * w + x] > 127) ? 1 : 0;
            rowsum += val;
            sum[(y + 1) * (w + 1) + (x + 1)] = rowsum + sum[y * (w + 1) + (x + 1)];
        }
    }
    int stride = w + 1;

    memset(bufs->visited, 0, vis_needed);
    uint8_t *visited = bufs->visited;

    int feat_len = db->feat_len;
    int max_cells = max_block / CELL;
    int n = 0, nt = 0;

    /* ── Tile spec array: records position of non-hero tiles ─────── */
    int spec_cap = bufs->spec_cap ? bufs->spec_cap : 512;
    TileSpec *specs = bufs->specs;
    if (!specs) {
        specs = (TileSpec *)malloc((size_t)spec_cap * sizeof(TileSpec));
        if (!specs) { *nout = 0; *ntiles = 0; return; }
        bufs->specs = specs;
        bufs->spec_cap = spec_cap;
    }
    int n_specs = 0;

    clock_t t0 = clock();

    /* ═══════════════════════════════════════════════════════════════
     * Phase 1: Serial greedy solver — identify all tiles
     * ═══════════════════════════════════════════════════════════════ */
    for (int cy = 0; cy < gh; cy++) {
        int y = cy * CELL;
        for (int cx = 0; cx < gw; cx++) {
            int x = cx * CELL;
            if (visited[(size_t)cy * gw + cx]) continue;
            int color = (gray[(size_t)y * w + x] > 127) ? 1 : 0;
            int mcw = 1, mch = 1;

            while (cx + mcw + 1 <= gw && mcw + 1 <= max_cells) {
                int any = 0;
                for (int yy = cy; yy < cy + mch; yy++)
                    if (visited[(size_t)yy * gw + (cx + mcw)]) { any = 1; break; }
                if (any) break;
                int x0 = cx * CELL, y0 = cy * CELL;
                int x1 = (cx + mcw + 1) * CELL, y1 = (cy + mch) * CELL;
                if (x1 > w) x1 = w;
                if (y1 > h) y1 = h;
                int64_t cnt = sum[y1 * stride + x1] - sum[y0 * stride + x1]
                        - sum[y1 * stride + x0] + sum[y0 * stride + x0];
                int area = (x1 - x0) * (y1 - y0);
                int pure = (color == 1) ? (cnt == area) : (cnt == 0);
                if (pure) mcw++; else break;
            }
            while (cy + mch + 1 <= gh && mch + 1 <= max_cells) {
                int any = 0;
                for (int xx = cx; xx < cx + mcw; xx++)
                    if (visited[(size_t)(cy + mch) * gw + xx]) { any = 1; break; }
                if (any) break;
                int x0 = cx * CELL, y0 = cy * CELL;
                int x1 = (cx + mcw) * CELL, y1 = (cy + mch + 1) * CELL;
                if (x1 > w) x1 = w;
                if (y1 > h) y1 = h;
                int64_t cnt = sum[y1 * stride + x1] - sum[y0 * stride + x1]
                        - sum[y1 * stride + x0] + sum[y0 * stride + x0];
                int area = (x1 - x0) * (y1 - y0);
                int pure = (color == 1) ? (cnt == area) : (cnt == 0);
                if (pure) mch++; else break;
            }

            int mw = mcw * CELL, mh = mch * CELL;
            if (x + mw > w) mw = w - x;
            if (y + mh > h) mh = h - y;
            for (int yy = cy; yy < cy + mch; yy++)
                for (int xx = cx; xx < cx + mcw; xx++)
                    visited[(size_t)yy * gw + xx] = 1;

            if (mw >= hero_min && mh >= hero_min) {
                manifest[n].x = x; manifest[n].y = y;
                manifest[n].w = mw; manifest[n].h = mh;
                manifest[n].op_id = (color == 1) ? -2 : -1;
                manifest[n].page_idx = -1;
                n++;
            } else {
                if (n_specs >= spec_cap) {
                    spec_cap *= 2;
                    TileSpec *tmp = (TileSpec *)realloc(specs, (size_t)spec_cap * sizeof(TileSpec));
                    if (!tmp) goto done;
                    specs = tmp;
                    bufs->specs = tmp;
                    bufs->spec_cap = spec_cap;
                }
                int midx = n;
                specs[n_specs].x = x;
                specs[n_specs].y = y;
                specs[n_specs].w = mw;
                specs[n_specs].h = mh;
                specs[n_specs].manifest_idx = midx;
                n_specs++;
                manifest[n].x = x; manifest[n].y = y;
                manifest[n].w = mw; manifest[n].h = mh;
                manifest[n].op_id = -1;
                manifest[n].page_idx = -1;
                n++;
            }
        }
    }

    if (n_specs == 0) goto done;

    /* ═══════════════════════════════════════════════════════════════
     * Phase 2: Pre-allocate feature buffers (pool bump-alloc)
     * ═══════════════════════════════════════════════════════════════ */
    TilePool pool;
    pool_init(&pool, (size_t)n_specs * feat_len + 4096);
    uint8_t *feat_bufs = pool_alloc(&pool, (size_t)n_specs * feat_len);
    if (!feat_bufs) { pool_free(&pool); goto done; }

    /* ═══════════════════════════════════════════════════════════════
     * Phase 3: Two-stage parallel feature extraction
     * ═══════════════════════════════════════════════════════════════ */
    clock_t tf = clock();
    int coarse_len = g_scales[0] * g_scales[0];
    if (n_specs > bufs->coarse_cap) {
        free(bufs->coarse_hit);
        bufs->coarse_cap = n_specs;
        bufs->coarse_hit = (int *)malloc((size_t)n_specs * sizeof(int));
    }
    int *coarse_hit = bufs->coarse_hit;
    if (!coarse_hit) { pool_free(&pool); goto done; }
    for (int i = 0; i < n_specs; i++) coarse_hit[i] = -1;
    {
#ifdef _OPENMP
        int nthreads = omp_get_max_threads();
#else
        int nthreads = 1;
#endif
        size_t max_crop_sz = (size_t)max_block * max_block * (db->channels == 3 ? 3 : 1);
        size_t crop_needed = (size_t)nthreads * max_crop_sz;
        if (crop_needed > bufs->crop_cap) {
            free(bufs->crop_bufs);
            bufs->crop_cap = crop_needed;
            bufs->crop_bufs = (uint8_t *)malloc(crop_needed);
        }
        uint8_t *crop_bufs = bufs->crop_bufs;
        if (!crop_bufs) { pool_free(&pool); goto done; }

#ifdef _OPENMP
        #pragma omp parallel
#endif
        {
#ifdef _OPENMP
            int tid = omp_get_thread_num();
#else
            int tid = 0;
#endif
            uint8_t *my_crop = crop_bufs + (size_t)tid * max_crop_sz;
            uint8_t *coarse_feat = (uint8_t *)malloc(coarse_len);

#ifdef _OPENMP
            #pragma omp for schedule(static)
#endif
            for (int i = 0; i < n_specs; i++) {
                TileSpec *sp = &specs[i];
                if (db->channels == 3) {
                    for (int yy = 0; yy < sp->h; yy++) {
                        memcpy(my_crop + (size_t)yy * sp->w * 3,
                               color_pixels + (size_t)(sp->y + yy) * color_stride + sp->x * 3,
                               (size_t)sp->w * 3);
                    }
                } else {
                    for (int yy = 0; yy < sp->h; yy++) {
                        memcpy(my_crop + (size_t)yy * sp->w,
                               gray + (size_t)(sp->y + yy) * w + sp->x,
                               (size_t)sp->w);
                    }
                }

                int N = g_scales[0];
                int sw = sp->w, sh = sp->h;
                int maxv = (1 << db->G) - 1;
                for (int dy = 0; dy < N; dy++) {
                    int sy0 = (int)((long long)dy * sh / N);
                    int sy1 = (int)((long long)(dy + 1) * sh / N);
                    if (sy1 > sh) sy1 = sh;
                    for (int dx = 0; dx < N; dx++) {
                        int sx0 = (int)((long long)dx * sw / N);
                        int sx1 = (int)((long long)(dx + 1) * sw / N);
                        if (sx1 > sw) sx1 = sw;
                        unsigned long psum = 0;
                        for (int sy = sy0; sy < sy1; sy++) {
                            for (int sx = sx0; sx < sx1; sx++) {
                                if (db->channels == 3) {
                                    const uint8_t *p = my_crop + ((size_t)sy * sp->w * 3 + sx * 3);
                                    psum += (29 * p[0] + 150 * p[1] + 77 * p[2]) >> 8;
                                } else {
                                    psum += my_crop[(size_t)sy * sw + sx];
                                }
                            }
                        }
                        int area = (sy1 - sy0) * (sx1 - sx0);
                        int v = (area > 0) ? (int)(psum / area) : 0;
                        int q = (db->G >= 8) ? v : (v * maxv + 127) / 255;
                        if (q > maxv) q = maxv;
                        coarse_feat[dy * N + dx] = (uint8_t)q;
                    }
                }

                int pid = -1;
                if (coarse_cache_lookup(coarse_feat, coarse_len, &pid)) {
                    coarse_hit[i] = pid;
                    continue;
                }

                Img crop;
                crop.w = sp->w; crop.h = sp->h;
                crop.channels = db->channels;
                crop.stride = sp->w * db->channels;
                crop.pixels = my_crop;
                img_compute_feature_multires(&crop, g_scales, g_n_scales,
                                              db->G, db->has_edges, (db->channels == 3),
                                               feat_bufs + (size_t)i * feat_len);
            }
            free(coarse_feat);
        }
    }

    /* Process coarse-cache hits (fast path — no full feature needed) */
    for (int i = 0; i < n_specs; i++) {
        if (coarse_hit[i] >= 0) {
            t->hits++;
            int midx = specs[i].manifest_idx;
            manifest[midx].op_id = coarse_hit[i];
            manifest[midx].page_idx = reg->entries[coarse_hit[i]].page_idx;
        }
    }
    t->feat += (double)(clock() - tf) / CLOCKS_PER_SEC;

    /* ═══════════════════════════════════════════════════════════════
     * Phase 4: Full-cache lookup for remaining (non-coarse-hit) tiles
     * ═══════════════════════════════════════════════════════════════ */
    if (n_specs > bufs->miss_cap) {
        free(bufs->miss_idx);
        bufs->miss_cap = n_specs;
        bufs->miss_idx = (int *)malloc((size_t)n_specs * sizeof(int));
    }
    int *miss_idx = bufs->miss_idx;
    if (!miss_idx) { pool_free(&pool); goto done; }
    nt = 0;
    for (int i = 0; i < n_specs; i++) {
        if (coarse_hit[i] >= 0) continue;
        const uint8_t *feat = feat_bufs + (size_t)i * feat_len;
        int pid = -1;
        if (cache_lookup(feat, &pid)) {
            t->hits++;
            int midx = specs[i].manifest_idx;
            manifest[midx].op_id = pid;
            manifest[midx].page_idx = reg->entries[pid].page_idx;
        } else {
            miss_idx[nt] = i;
            memcpy(tiles + (size_t)nt * feat_len, feat, (size_t)feat_len);
            nt++;
        }
    }

    /* ═══════════════════════════════════════════════════════════════
     * Phase 5: Batch match + cache_put (both coarse + full)
     * ═══════════════════════════════════════════════════════════════ */
    if (nt > 0) {
        clock_t tm = clock();
        if (nt > bufs->result_cap) {
            free(bufs->results);
            bufs->result_cap = nt;
            bufs->results = (int *)malloc((size_t)nt * sizeof(int));
        }
        int *results = bufs->results;
        if (!results) { pool_free(&pool); goto done; }
        int coarse_for_match = g_scales[0] * g_scales[0];
        match_batch_coarse(db->data, tiles, db->n_pages, nt, feat_len, coarse_for_match, results);
        for (int i = 0; i < nt; i++) {
            int pid = results[i];
            int midx = specs[miss_idx[i]].manifest_idx;
            manifest[midx].op_id = pid;
            manifest[midx].page_idx = reg->entries[pid].page_idx;
            cache_put(tiles + (size_t)i * feat_len, pid);
            coarse_cache_put(tiles + (size_t)i * feat_len, coarse_len, pid);
        }
        t->match += (double)(clock() - tm) / CLOCKS_PER_SEC;
        t->tiles += nt;
    }

    pool_free(&pool);
done:
    t->solve += (double)(clock() - t0) / CLOCKS_PER_SEC;
    *nout = n;
    *ntiles = nt;
}
